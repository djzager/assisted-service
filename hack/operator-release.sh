...
COMMUNITY_OPERATORS_REPO="operator-framework/community-operators"
...
function co_get_operator_versions() {
    ...
    PREV_OPERATOR_VERSION=$(c=${channel} yq eval --exit-status \
        ...
        ${COMMUNITY_OPERATORS_DIR}/community-operators/assisted-service-operator/assisted-service.package.yaml | \
...
function co_update_manifests() {
    ...
    cp -r ${MANIFESTS_DIR} ${COMMUNITY_OPERATORS_DIR}/community-operators/assisted-service-operator/${OPERATOR_VERSION}

    # Update the CSV
    CO_CSV="${COMMUNITY_OPERATORS_DIR}/community-operators/assisted-service-operator/${OPERATOR_VERSION}/${CSV}"
...
function co_submit_pr() {
    ...
    gh pr create \
      ...
      --body "$(curl -sSLo - https://raw.githubusercontent.com/operator-framework/community-operators/master/docs/pull_request_template.md | \
        sed -r -n '/#+ Updates to existing Operators/,$p' | \
        sed -r -e 's#\[\ \]#[x]#g')"
    popd
    rm -r ${COMMUNITY_OPERATORS_DIR}
}

# function assisted_update_manifests() {
#     pushd ${ASSISTED_SERVICE_DIR}
#     git checkout -B ${OPERATOR_VERSION}
#     version="assisted-service-operator.v${OPERATOR_VERSION}" \
#         yq eval --exit-status --inplace \
#         '.spec.skips += strenv(version)' \
#         ${ASSISTED_SERVICE_DIR}/config/manifests/bases/assisted-service-operator.clusterserviceversion.yaml
...
#     gh pr create \
#       --draft \
#       --base upstream:${ASSISTED_BRANCH} \
#       --title "$(git log -1 --format=%s)" \
#       --body "Update to operator manifests"
#     popd
#     rm -r ${ASSISTED_SERVICE_DIR}
# }
